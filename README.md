...
| `--ddlHandling`                         | Either `failAll` (default) or `warnMost`. Under `failAll` any DDL change events will crash the verifier. `warnMost` makes the verifier instead show a warning and skip the event. (In this case you **MUST** manually confirm that the change is replicated to the destination.) |
